<template>
  <el-card shadow="never">
    ...
    <el-table :data="tableData"
      ...
      <el-table-column prop="createTime" label="评论时间" align="center" min-width="140px"></el-table-column>
      <el-table-column prop="text" label="留言内容" align="center" min-width="300px"></el-table-column>
...
export default {
  ...
  methods: {
    // 初始化表格信息
    init() {
      this.request.get("/feedback/admin/list", {
        params: {
    ...
    // 删除
    handleDelete(id) {
      this.request.delete("/feedback/admin/delete/" + id).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/feedback/admin/delete/batch", ids).then(res => {
        if (res.code === '200') {
